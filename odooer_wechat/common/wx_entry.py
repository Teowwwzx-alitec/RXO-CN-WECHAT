...
class WxEntry(EntryBase):
    ...
    def init(self, env, from_ui=False):
        ...
        self.subscribe_auto_msg = config.reply

        self.client = WeChatClient(self.wx_appid, self.wx_secret, session=self.gen_session())
        ...
        #     for r in users:
        #         if r.last_uuid_time:
        #             self.recover_uuid(r.openid, r.last_uuid, fields.Datetime.from_string(r.last_uuid_time))
        # except:
        #     env.cr.rollback()
